  ...
  $.widget('ra.filteringSelect', {
    ...
    _create: function() {
      ...
      if (filtering_select.length > 0) {
        ...
        this.element.hide();
        filtering_select = this._inputGroup(this.element, this.element.data('noSm'));
    ...
    _buttonField: function() {
      return $(
        '<span class="input-group-btn">' +
          '<label class="btn btn-info dropdown-toggle btn-table" data-toggle="dropdown" aria-expanded="false" title="Show All Items" role="button">' +
    ...
    _inputGroup: function(inputFor, nosm = false) {
      var x = $('<div>')
        .addClass('input-group filtering-select')
        .attr('data-input-for', inputFor.attr('id'))
        .css('float', 'left');
      if(!nosm)
        x.addClass('col-sm-2');
      return x;
